...
public class ProductServiceImpl implements ProductService {
    ...
    @Transactional
    //1.若缩略图参数有值，则处理缩略图,
    //2.若原先存在缩略图则先删除再添加新图,之后获取缩略图的相对路径并复制给product
    //若商品详情图列表参数有值,对商品详情图片列表进行同样的操作
    //3.将tb_product_img下面的该商品原先的商品详情图记录全部清除
    //4.更新tb_product_img,tb_product信息
    ...
    private void addProductImgList(Product product,List<ImageHolder> productImageHolderList){
        //获取图片存储路径,这里直接存储到相应店铺的文件夹底下
        String dest=PathUtil.getShopImagePath(product.getShop().getShopId());
        List<ProductImg> productImgList=new ArrayList<ProductImg>();
        //遍历图片一次取处理，并添加到productImg实体类里
